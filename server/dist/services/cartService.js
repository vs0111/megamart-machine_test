...
const cartModel_js_1 = require("../models/cartModel.js");
...
class CartService {
    ...
    async getCart(userId) {
        let cart = await cartModel_js_1.CartModel.findOne({ userId });
        if (!cart) {
            cart = await cartModel_js_1.CartModel.create({ userId, items: [] });
        }
        const hydratedItems = [];
        const alerts = [];
        let hasStaleItems = false;
        for (const item of cart.items) {
            const product = await productModel_js_1.ProductModel.findById(item.productId);
            if (!product) {
        ...
        const originalTotal = validItems.reduce((acc, item) => acc + (item.variant.originalPrice || item.variant.price) * item.quantity, 0);
    ...
    async addItem(userId, productId, variantId, quantity = 1) {
        const product = await productModel_js_1.ProductModel.findById(productId);
        if (!product) {
    ...
    async checkout(userId, shippingDetails) {
        ...
        try {
            ...
            for (const item of cartData.items) {
                const updatedProduct = await productModel_js_1.ProductModel.findOneAndUpdate({
                    _id: item.productId,
            ...
            for (const ded of successfulDeductions) {
                await productModel_js_1.ProductModel.findOneAndUpdate({ _id: ded.productId, 'variants.id': ded.variantId }, { $inc: { 'variants.$.stock': ded.quantity }, $set: { 'variants.$.inStock': true } });
            }
            throw error;
        }
        ...
        const orderDoc = await orderModel_js_1.OrderModel.create({
            orderId,
            userId,
